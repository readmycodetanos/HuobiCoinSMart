 ...
 */
package com.starstudio.frame.net.utils;
...
public class HeaderParser {
     ...
     *
     * @param abstractRequest     请求类
     * @param cacheEntity 缓存实体类
     * @param cacheMode   缓存模式
     */
    public static <T> void addCacheHeaders(RequestAbstract abstractRequest, CacheEntity<T> cacheEntity, CacheMode cacheMode) {
        ...
        if (cacheEntity != null && cacheMode == CacheMode.DEFAULT) {
            ...
            if (responseHeaders != null) {
                ...
                if (eTag != null) abstractRequest.headers(HttpHeaders.HEAD_KEY_IF_NONE_MATCH, eTag);
                long lastModified = HttpHeaders.getLastModified(responseHeaders.get(HttpHeaders.HEAD_KEY_LAST_MODIFIED));
                if (lastModified > 0) abstractRequest.headers(HttpHeaders.HEAD_KEY_IF_MODIFIED_SINCE, HttpHeaders.formatMillisToGMT(lastModified));
            }
        }
    }
}
